...
def main():
    ...
    exps = read_test("exps/trte_rvrt_deblur/test.cfg",
                     ".cache_io_exps/trte_rvrt_deblur/test",reset=refresh,skip_dne=refresh)
    exps,uuids = cache_io.get_uuids(exps,".cache_io/trte_rvrt_deblur/test",
                                    reset=refresh,no_config_check=refresh)
    print(len(exps))

    # -- run exps --
    results = cache_io.run_exps(exps,test.run,uuids=uuids,
                                name=".cache_io/trte_rvrt_deblur/test",
                                version="v1",skip_loop=False,clear_fxn=clear_fxn,
                                clear=False,enable_dispatch="slurm",
                                records_fn=".cache_io_pkl/trte_rvrt_deblur/test.pkl",
                                records_reload=False,to_records_fast=True,
                                proj_name="test_rvrt_deblur")
