...
class GraphCut(th.nn.Module):
    ...
    def forward(self,vid,anno_gt):
        ...
        weights = th.exp(-10*dists.detach())#th.nn.functional.softmax(dists,-1).detach()

        # -- optimize assignment --
        # anno_W = (th.rand_like(vid[...,[0],:,:])-0.5).requires_grad_(True)
        noise = 0.1*(th.rand_like(vid[...,[0],:,:])-0.5)
        anno_W = (noise + 0.000*(anno_gt*2-1)).requires_grad_(True)
        optim = th.optim.Adam([anno_W],lr=self.lr,weight_decay=0.)
        # optim = th.optim.SGD([anno_W],lr=self.lr)
        lamb_reg = 0.2
        for i in range(self.niters):
            ...
            if i >= 3000 and False:
                ...
                # exit()
                # ws,wt,k,ps = self.ws,self.wt,self.k,self.ps
                # ws,wt,k,ps = self.ws,self.wt,self.k//2,self.ps
                # ws,wt,k,ps = self.ws,self.wt,self.k,self.ps
                # search = stnls.search.NonLocalSearch(ws,wt,ps,k,
                #                                      nheads=1,dist_type="l2",
                #                                      stride0=self.stride0,
                ...
                #                                      itype="float")
                # dists,flows_k = search(vid,vid,flows)
                # weights = th.exp(-2*dists.detach())
                # anno_W = (anno_W.detach() + (th.rand_like(anno_W)-0.5)*0.001).requires_grad_(True)
                # optim = th.optim.Adam([anno_W],lr=self.lr,weight_decay=0.)
                # # dists,flows_k = search(vid,vid,flows)
                # # weights = th.exp(-2*dists.detach())
                # lamb_reg = 0.001
            ...
            # -- compute min-cut --
            anno = th.nn.functional.tanh(anno_W)
            l2_dists,_ = refine(anno,anno,flows_k)
            loss = th.mean(weights * l2_dists)
            loss += lamb_reg*th.mean((1-anno.abs())**2)
            loss.backward()
            optim.step()

            # -- info --
            # print(anno.mean().item(),anno.min().item(),anno.max().item())
            # print(anno_gt.mean().item(),anno_gt.min().item(),anno_gt.max().item())
            anno_eval = (anno+1)/2.
            psnr = compute_psnrs(anno_eval,anno_gt,div=1.)
            print("[loss,psnr]: %2.2e %2.2f" % (loss.item(),psnr.mean().item()))

        # -- return optimal value --
        return (anno+1)/2.

    def slic(self,vid,anno_gt):
        igrads = img_grads(vid)
