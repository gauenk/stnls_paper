...
def error_and_time(records):
    ...
    fields = ['dtime','exact_time']
    for field in fields:
        records[field] = np.log10(records[field])
    ...
    # -- colors to nbwd --
    cmap = LinearSegmentedColormap.from_list("",["blue","orange"])
    ...
    fn = str(root / "error_and_time.png")
...
def time_vs_channels(records):
    ...
    root = Path(str(SAVE_DIR))
    if not root.exists(): root.mkdir(parents=True)
    fn = str(root / "time_vs_channels.png")
...
def errors_vs_channels(records):
    ...
    root = Path(str(SAVE_DIR))
    if not root.exists(): root.mkdir(parents=True)
    fn = str(root / "errors_vs_channels.png")
    print("Saving figure %s" % fn)
    plt.savefig(fn,dpi=800,transparent=True)
